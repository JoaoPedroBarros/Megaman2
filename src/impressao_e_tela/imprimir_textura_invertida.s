##############################################################
# PROC_IMPRIMIR_TEXTURA_INVERTIDA		             #
# Imprime uma textura de tamanho variavel em uma coordenada  #
# X Y, ignorando pixeis invisiveis e pulando coordenadas     #
# fora da tela, imprimindo invertido.                        #
# 							     #
# ARGUMENTOS:						     #
...
# a7 = X_limite_tela  (X minimo para estar fora da tela)
			...
			li a7, LARGURA_VGA
                        ...
                        bnez t2, P_IT3_LOOP_RAPIDO      # se a textura estah COMPLETAMENTE VISIVEL, vamos pro loop rapido
			j P_IT3_LOOP			# senao, vai pro loop normal
...
P_IT3_LOOP:		lbu t0, (a0)			# coloca a informacao do pixel em I (I = informacao em P)
			
                        blt t3, a5, P_IT3_PULA_PIXEL 	# SE (P < MINIMO), PULA O PIXEL
                        bge t3, a6, P_IT3_PULA_PIXEL    # SENAO SE (P >= MAXIMO), TERMINA IMPRESSAO
			bltz a1,    P_IT3_PULA_PIXEL	# SENAO SE (X < 0), PULA O PIXEL
                        bge a1, a7, P_IT3_PULA_PIXEL    # SENAO SE (X >= X_LIMITE), PULA O PIXEL
			beq t0, t6, P_IT3_PULA_PIXEL	# SENAO SE ( I == COR_TRANSPARENTE ), PULA O PIXEL
                                                        # SENAO:
			sb t0,	(t3)			# 	imprime o pixel
P_IT3_PULA_PIXEL:	addi a0, a0, -1			# E--
			addi t3, t3, 1			# P++ 
			addi a1, a1, 1			# X++ 
			
			# SE X == X_limite, VAI PARA A PROXIMA LINHA
			bne a1, t4, P_IT3_LOOP
			j P_IT3_PROXIMA_LINHA



P_IT3_PROXIMA_LINHA_RAPIDO:    
...
P_IT3_LOOP_RAPIDO:	lbu t0, (a0)			# coloca a informacao do pixel em I (I = informacao em P)
			
			beq t0, t6, P_IT3_PULA_PIXEL_RAPIDO# SE ( I == COR_TRANSPARENTE ), PULA O PIXEL
                                                        # SENAO:
			sb t0,	(t3)			# 	imprime o pixel
P_IT3_PULA_PIXEL_RAPIDO:addi a0, a0, -1			# E--
			addi t3, t3, 1			# P++ 
			addi a1, a1, 1			# X++ 
			
			bne a1, t4, P_IT3_LOOP_RAPIDO
			# SE X == X_limite, VAI PARA A PROXIMA LINHA
			j P_IT3_PROXIMA_LINHA_RAPIDO		
